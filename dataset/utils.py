...
def collect_result(result, result_dir, filename, is_json=True, is_list=True):
    if is_json:
        ...
        mindspore.save_checkpoint(result,result_file)
    # 由于是单卡环境，因此不需要等待同步
    #dist.barrier()
    ...
    if utils.is_main_process():   
        ...
        for rank in range(utils.get_world_size()):
            if is_json:
                ...
                res = mindspore.load_checkpoint(result_file)
            if is_list:
                result += res
            else:
                result.update(res) 
...
def save_result(result, result_dir, filename, is_json=True, is_list=True):
    ...
    if utils.is_main_process():   
        ...
        if is_json:                  
            ...
            mindspore.save_checkpoint(result, final_result_file)
...
def grounding_eval(results,dets,cocos,refer,alpha,mask_size=24):
    ...
    for res in tqdm(results):
        ...
        # 功能一致, 但在’bicubic’模式 align_corners=False 时，计算方式和TensorFlow相同，结果和PyTorch有差异
        mask = interpolate(mask,size = (image['height'],image['width']), mode='bicubic').squeeze()
